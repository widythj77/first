...
print(PROJECT_DIR)
# C:\Users\student\Desktop\BD\first
...
env.key_filename   = 'first.pem' # 동일 폴더 위치
...
def _setup() :
    ...
    # 상호 프로젝트간 영향을 받지 않게 만드는 방식(가상환경)
...
# 7-2. apt(우븐트상) 패키지를 업데이트 여부 확인 및 업데이트(진행여부)
def _init_apt()
...
def update():
    ...
    _virtualhost_make()
    # 8-4. 아파치가 프로젝트를 access 함수 있게 권한 처리 (1회만수행)
...
# 8-4. 아파치 서버가 파이썬 웹을 엑세스 할수 있게 처리
def _grant_apache():
